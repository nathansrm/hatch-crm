---
title: REST API
description: This document explains how to use the Hatch CRM API outside of the Supabase data provider.
---

Hatch CRM exposes a complete REST API that lets you do everything you can do with the CRM.
...
Use the Supabase Studio to see the [automatically generated documentation](https://supabase.com/dashboard/project/_/api) for your Hatch CRM instance. It lists all the available endpoints and their parameters. This page explains how to call each endpoint either with the supabase client, or bash (curl).
...
Here is the list of all the Hatch CRM specific endpoints:
...
<a id="get-one-note"></a>
All GET query in Postgrest will always returns a list of items by default. To
have the query returns a single item directly, you can add the Accept header
with the value `Accept: application/vnd.pgrst.object+json` to your request. :::
...
```js
async function addMessageAsNoteToContact({ message, email, password, apikey }) {
    // Step 1: Authenticate the user of the widget and get a JWT token
    const authResponse = await fetch(
        'https://<instance_url>/auth/v1/token?grant_type=password',
        {
            method: 'POST',
    ...
    const contactsResponse = await fetch(
        `https://<instance_url>/rest/v1/contacts_summary?email_fts=ilike.*${encodeURIComponent(
            message.email,
        )}*`,
        {
            headers: {
