...
description: This document explains how to configure inbound email processing in Hatch CRM.
---
...
Hatch CRM uses [Postmark](https://postmarkapp.com/) to receive and process inbound emails.
...
Hatch CRM uses a Supabase Edge Function to handle the webhook and process the received emails. You need to configure your Supabase project to handle the incoming requests.

In a terminal, after [linking your Supabase project](../supabase-configuration#alternative-using-an-existing-supabase-instance), run the following commands:
...
:::tip
You need to provide `VITE_INBOUND_EMAIL` at two places: as a Supabase Secret ([Supabase Setup](#supabase-setup)), and in the `.env` file used by the Vite app (this section).
This is necessary because it is used both client-side, to display the Inbound Email in the Hatch CRM app, and server-side (by the Edge Function) to detect when an email is forwarded to Hatch CRM.
:::
