...
description: This document explains how to configure SSO for Hatch CRM.
---

Hatch CRM supports SSO (Single Sign-On) authentication through SAML 2.0. This allows your users to authenticate using their existing credentials from an identity provider, such as Google Workspace, Okta, or Microsoft Azure AD.
...
3. **Fill out app details**
    ...
    The information you enter here is for visibility into your Google Workspace. You can choose any values you like. _Hatch CRM_ as a name works well for most use cases. Optionally enter a description and a logo.
...
6. **Configure attribute mapping**
    ...
    To properly map the identity details to the sales fields in Hatch CRM, add the following mappings:
...
7. **Configure user access**
    ...
    You can configure which Google Workspace user accounts will get access to Hatch CRM. This is important if you wish to limit access to your software engineering teams.
...
Then, run the following command where the `metadata.xml` file is the file you downloaded at _step 4_ and `mapping.json` the file you created just now. Replace `<your-project>` with your Supabase project identifier and `company.com` with your company domain:
...
The login page will now show a button to log in with Google Workspace SSO.

![Login page with Google Workspace SSO](../../images/sso-google.png)

When users click on this button, they will be redirected to the Google login page. After they log in, they will be redirected back to Hatch CRM and logged in automatically.
