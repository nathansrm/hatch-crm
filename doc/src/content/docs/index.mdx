...
description: Hatch CRM is designed to be easy to customize and extend by developers with basic React and SQL skills.
---

Hatch CRM is designed to be easy to customize and extend by developers with basic React and SQL skills. This guide will help you get started with installing, configuring, and deploying Hatch CRM.
...
1. Fork the Hatch CRM repository on GitHub to your own account.

    ![Fork the repository](../images/hatch-crm-repository.png)

2. Clone your forked repository to your local machine:

    ```bash
    git clone https://github.com/[username]/hatch-crm.git
    cd hatch-crm
    ```
...
Hatch CRM now runs at [http://localhost:5173/](http://localhost:5173/).
...
It's easier to develop on a non-empty application. Hatch CRM provides a sample dataset you can import to get started. Click the import button and choose the sample data file at `test-data/contacts.csv`. This will add 500 contacts and 55 companies to your local database.
...
The entry point of the application is the `src/App.tsx` file. By default, this file simply renders the `<CRM>` component, which is the root component of Hatch CRM.
...
Check the [Customizing Hatch CRM](./developers/customizing) guide for an exhaustive list of configuration settings you can customize.

## Adding Custom Fields

You can extend the data model of Hatch CRM by adding custom fields to the existing entities (Contacts, Companies, Deals, Notes, and Tasks).
...
2. Choose the Tables section in the Database management panel. You will see the list of tables used by Hatch CRM.
...
Hatch CRM uses views to simplify the queries. You need to update the `contacts_summary` view to include the new `referred_by` field.
...
The frontend code lives in the `src/components/hatch-crm` folder, and is organized by entities. Each entity has its own folder containing all the components related to that entity. Take a few minutes to explore the codebase and familiarize yourself with its structure.

:::tip
Hatch CRM leverages two libraries that provided high-level building blocks: [Shadcn Admin Kit](https://marmelab.com/shadcn-admin-kit/) for the application logic, and [Shadcn/ui](https://ui.shadcn.com/) for the UI components. The code for these libraries is copied in the `src/components` folder.
:::

Hatch CRM uses TypeScript to ensure type safety, so you'll need to modify the `Contact` type to include the new `referred_by` field. Open the `src/components/hatch-crm/types.ts` file and add the `referred_by` field to the `Contact` interface.
...
To do so, open the `src/components/hatch-crm/contacts/ContactInputs.tsx` file to edit the `<ContactInputs>` component and add a new `<TextInput>`.
...
Finally, update the Contacts sidebar to display the referrer when available. Open the `src/components/hatch-crm/contacts/ContactAside.tsx` file and add a new `<TextField>` for the `referred_by` field.
...
It's time to deploy your customized version of Hatch CRM to production. You use [Supabase.com](https://supabase.com) to host the backend, and GitHub Pages to host the frontend. Both services offer free plans that are sufficient for Hatch CRM.

First, login or register to [Supabase.com](https://supabase.com).

Hatch CRM comes with a CLI utility to create a backend instance on Supabase.com:
...
The frontend of the CRM is a Single-Page App that can be deployed to any CDN. Since you have forked the Hatch CRM repository on GitHub, you can use GitHub Pages to host the frontend.
...
After a couple minutes, your customized CRM will be available at `https://<username>.github.io/hatch-crm/`, and connected to the production Supabase instance you created earlier.

You can proceed with the creation of the first admin user and start using your CRM in production!

![Hatch CRM in Production](../images/hatch-crm-demo.png)

:::tip
Supabase.com and GitHub Pages are just two of the many services you can use to host your Hatch CRM. You can [self-host a supabase instace](https://supabase.com/docs/guides/self-hosting), and use any static server or CDN (e.g. CloudFlare, Netlify, Vercel, etc.) to host the frontend.
:::

## Next Steps

Explore the documentation to learn more about Hatch CRM:
