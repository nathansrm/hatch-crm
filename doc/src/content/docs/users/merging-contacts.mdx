...
Hatch CRM allows you to merge duplicate contacts into a single contact to maintain a clean and organized database.
...
Hatch CRM will merge the selected contact into the target contact. This means that all information from the selected contact will be transferred to the target contact, and the selected contact will be deleted.

When both the current contact and the target contact have a given property (like position or company), Hatch CRM keeps the one of the target contact.
