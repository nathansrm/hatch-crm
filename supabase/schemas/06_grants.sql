...
-- Security: anon role has minimal access. authenticated gets table/function access.
-- service_role gets full access for edge functions and server-side operations.
--
...
--
-- Function grants
-- Only authenticated and service_role get function access.
-- anon is excluded from all functions.
-- Stale grants for dropped functions (get_avatar_for_email, get_domain_favicon) removed.
--
...
--
-- Table grants
-- anon gets no table access. authenticated gets all. service_role gets all.
--
...
--
-- Default privileges
-- Future objects get authenticated + service_role access only. No anon defaults.
--
