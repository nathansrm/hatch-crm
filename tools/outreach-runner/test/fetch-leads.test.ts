...
async function runWithRows(
  ...
  steps: unknown[],
  max = '10'
): Promise<{ code: number; stdout: string[]; stderr: string[]; calls: TableCalls }> {
  const calls: TableCalls = {};
  const createClient = () => ({
    from(table: string) {
      calls[table] = [];
      const result = table === 'intake_leads' ? { data: leads, error: null } : { data: steps, error: null };
      return makeBuilder(result, calls[table]);
    },
  });
  const moduleMock = mock as ModuleMock;

  const mod = await import(`../src/fetch-leads.ts?case=${Date.now()}-${Math.random()}`);
...
test('happy path returns flattened lead rows', async () => {
  ...
  assert.deepEqual(calls.intake_leads.map((call) => call.method), [
    'select',
    'eq',
    'not',
    'neq',
  ...
  assert.deepEqual(calls.outreach_steps.map((call) => call.method), ['select', 'in']);
});
