...
function resultFor(
  body: string,
  subject = "Operations audit for Acme Plumbing",
) {
  return validate({ subject, body }, lead);
}

test("valid 88-word body with Acme Plumbing in body passes", () => {
  assert.deepEqual(resultFor(bodyWith("Acme Plumbing")), {
    pass: true,
    reasons: [],
  });
});
...
test("wrong subject prefix reports subject mismatch", () => {
  const result = resultFor(
    bodyWith("Acme Plumbing"),
    "Audit for Acme Plumbing",
  );

  assert.equal(result.pass, false);
  assert.ok(
    result.reasons.includes(
      "subject must be exactly 'Operations audit for Acme Plumbing'",
    ),
  );
});
...
test("right subject prefix with wrong company reports subject mismatch", () => {
  const result = resultFor(
    bodyWith("Acme Plumbing"),
    "Operations audit for Beta Plumbing",
  );
...
test("validateRaw with invalid JSON reports invalid JSON", () => {
  assert.deepEqual(validateRaw("not-json"), {
    pass: false,
